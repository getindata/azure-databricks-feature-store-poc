...
# Authentication to Databricks workspace using AAD token
provider "databricks" {
...
data "local_file" "aad_token_file" {
...
resource "azurerm_databricks_workspace" "dbx" {
  ...
  location            = azurerm_resource_group.rg.location
  sku                 = "premium"
  # customizing storage_account_name here didn't work - resulted in a broken DBX deployment
  #   custom_parameters {
  #     storage_account_name = "dbfsstorage"
  #   }
}
...
# CosmosDB can be used as a backend for online feature store for Databricks ML Runtime > 11.1 (https://docs.databricks.com/dev-tools/api/python/latest/feature-store/online_store_spec/azure_cosmosdb_online_store_spec.html)
...
resource "azurerm_cosmosdb_sql_database" "dbfs" {
  name                = "feature_store_taxi_example"
  resource_group_name = azurerm_resource_group.rg.name
  account_name        = azurerm_cosmosdb_account.cdbacc.name
}


resource "azurerm_cosmosdb_sql_container" "fs1" {
  name                  = "trip_pickup_features"
  resource_group_name   = azurerm_resource_group.rg.name
  account_name          = azurerm_cosmosdb_account.cdbacc.name
  database_name         = azurerm_cosmosdb_sql_database.dbfs.name
  partition_key_path    = "/definition/yyyy_mm"
  partition_key_version = 1
  throughput            = 400
...
resource "azurerm_cosmosdb_sql_container" "fs2" {
...
# Installing connector library for Databricks -> Cosmos DB
...
resource "azurerm_key_vault_secret" "stg-access-key" {
  name         = "stg-access-key"
...
resource "databricks_notebook" "notebook-3" {
  source = format("${path.module}/databricks_notebooks/%s", var.notebook_3)
  path   = format("${data.databricks_current_user.me.home}/terraform/%s", var.notebook_3)
}
